...
import { IBlogItem } from '../Interface/IProps'

const BlogItem: React.FC<IBlogItem> = ({ id, name, title, userId, views, comments, img, tags, getByTag, colorUser }) => {
  return (
    <div className={styles.postItem}>
      ...
      <div className={styles.info}>
        ...
        <div>
          ...
          {
            tags
              ? tags.map((tag: string, i: number) => i < 8 ? <p className={styles.tagItem} key={String(tag + i)} onClick={() => getByTag(tag)}>#{tag}</p> : null)
              : null
          }
