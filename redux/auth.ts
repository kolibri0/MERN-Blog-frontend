...
export const followOnUser = createAsyncThunk(
  'user/followOnUser',
  async (data: follow) => {
    const res = await axios.patch(`/user/${data.myId}/follow/${data.userId}`)
    return res.data
  }
)

export const deleteFollowOnUser = createAsyncThunk(
  'user/deleteFollowOnUser',
  async (data: follow) => {
    const res = await axios.delete(`/user/${data.myId}/follow/${data.userId}`)
    return res.data
  }
)
...
const userSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    //---------------------------------------------------------------------
    builder.addCase(followOnUser.fulfilled, (state, action) => {
      state.user = action.payload
    })
    //---------------------------------------------------------------------
    builder.addCase(deleteFollowOnUser.fulfilled, (state, action) => {
      state.user = action.payload
    })
  }
})
